...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if( ! $erreur )
	{
		...
		$req = $db->prepare('INSERT INTO dsc_projet.Pompier (Matricule, NomPompier, PrenomPompier, DateNaissPompier, TelPompier, SexePompier, idGrade) VALUES (:mMatricule, :mNomPompier, :mPrenomPompier, :mDateNaissPompier, :mTelPompier, :mSexePompier, :midGrade )');
		...
		catch(PDOException $e)
		{
			if ($e->getCode()==23000)
			{
                ...
                header('Location: ./formajoutpompier.php'); // On redirige
		...
		$pro = $db->prepare('INSERT INTO dsc_projet.Professionnel (MatPro, DateEmbauche, Indice) VALUES (:mMatricule, :mDateEmbauche, :mIndice)');

		// Préparation de la requête pour le pompier Volontaire
		$vol = $db->prepare('INSERT INTO dsc_projet.Volontaire (MatVolontaire, Bip, IdEmployeur) VALUES (:mMatricule, :mBip, :mIdEmployeur)');
		...
		{
			echo "pbm dans l'ajout des tables volontaire ou professionnel !";
			echo $e->getMessage();
		}

		// Ajout dans affectation
		$req = $db->prepare('INSERT INTO dsc_projet.Affectation (Matricule, DateAff, IdCaserne) VALUES (:mMatricule, :mDateAffectation, :mIdCaserne)');
		$req->bindParam(':mMatricule', $matricule);
		$dateJour = date("Y-m-d");
		...
		{
			echo "pbm dans l'ajout d'une valeur dans la table Affectation !";
			echo $e->getMessage();
            header('Location: ./formajoutpompier.php'); // On redirige
